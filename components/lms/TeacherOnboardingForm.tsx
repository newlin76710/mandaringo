...
export function TeacherOnboardingForm() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<TeacherProfileInput>({ resolver: zodResolver(teacherProfileSchema), defaultValues: { gender: "FEMALE" } });

  async function onSubmit(profile: TeacherProfileInput) {
    setSubmitting(true);
    const result = await completeOnboarding({ role: "TEACHER", profile });
    setSubmitting(false);
    if (result.error) {
