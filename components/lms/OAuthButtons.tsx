...
> = {
  ...
  line: {
    label: "使用 LINE 繼續",
    icon: LineIcon,
    className: "bg-[#06C755] text-white hover:bg-[#05B04D]",
  },
  facebook: {
...
export function OAuthButtons({ providers, callbackUrl }: { providers: string[]; callbackUrl: string }) {
      ...
      {providers.map((p) => {
        ...
        return (
          <Button
            ...
            className={`h-12 w-full justify-center text-base font-semibold ${config.className}`}
          >
            {loading === p ? <Loader2 className="h-5 w-5 animate-spin" /> : <Icon />}
            {config.label}
          </Button>
        );
      })}
